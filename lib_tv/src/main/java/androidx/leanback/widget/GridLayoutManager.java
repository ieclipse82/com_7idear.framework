...
import com_7idear.tv.utils.FocusFinderUtils;
...
final class GridLayoutManager extends RecyclerView.LayoutManager {
    ...
    private View mLastFocusRow; //最后的焦点行
    private View mLastFocusView; //最后的焦点视图
    private int  mLastFocusDirection; //最后的焦点移动方向
    ...
    abstract class GridLinearSmoothScroller extends LinearSmoothScroller {
        ...
        protected void onStopInternal() {
            ...
            //当最后一次的焦点视图不是真实焦点视图时，重设焦点视图
            //            if (hasFocus()) {
            //                mFlag |= PF_IN_SELECTION;
            //                targetView.requestFocus();
            //                mFlag &= ~PF_IN_SELECTION;
            //            }
            if (mLastFocusView != targetView.findFocus()) {
                setNewFocusViewAfterLayout(targetView);
            }
    ...
    final class PendingMoveSmoothScroller extends GridLinearSmoothScroller {
        ...
        void consumePendingMovesBeforeLayout() {
            ...
            //获取快速滑动时焦点行
            mLastFocusRow = findViewByPosition(mFocusPosition);
        ...
        public PointF computeScrollVectorForPosition(int targetPosition) {
            ...
            if (mOrientation == HORIZONTAL) {
                //设置最后一次的焦点方向（水平）
                mLastFocusDirection = direction == 1 ? View.FOCUS_RIGHT : View.FOCUS_LEFT;
                return new PointF(direction, 0);
            } else {
                //设置最后一次的焦点方向（垂直）
                mLastFocusDirection = direction == 1 ? View.FOCUS_DOWN : View.FOCUS_UP;
    ...
    /**
     * 设置新焦点视图在测绘显示布局之后 {{@link BaseGridView}}
     * @param v 当前布局视图
     */
    private void setNewFocusViewAfterLayout(View v) {
        View newFocus = null;
        //最后的焦点行不为空时在当前布局中获取最佳焦点单元
        if (mLastFocusRow != null && v instanceof ViewGroup) {
            View focused = mLastFocusRow.findFocus();
            /**
             * 列表中有{@link BaseGridView}时，getFocusScrollStrategy()不为FOCUS_SCROLL_ITEM时
             * 会导致获取焦点乱跳或跳到首个视图的问题，推荐设置焦点在末尾的属性解决（FOCUS_SCROLL_ITEM）
             * 否则可开启下面的代码块自己查找解决
             */
            //            ViewGroup baseGridView = findBaseGridView(v);
            //            if (baseGridView != null) {
            //                newFocus = FocusFinderUtils.findNextFocus(mBaseGridView, baseGridView, focused,
            //                        mLastFocusDirection, true);
            //            } else {
            //                newFocus = FocusFinderUtils.findNextFocus(mBaseGridView, (ViewGroup) v, focused,
            //                        mLastFocusDirection);
            //            }
            newFocus = FocusFinderUtils.findNextFocus(mBaseGridView, (ViewGroup) v, focused,
                    mLastFocusDirection);
        ...
        //如果没有最佳焦点单元，设置当前布局获取焦点
        if (newFocus == null) newFocus = v;
        if (newFocus != null && hasFocus()) {
            mLastFocusView = newFocus;
            mFlag |= PF_IN_SELECTION;
    ...
    public View onInterceptFocusSearch(View focused, int direction) {
        ...
        if (result != null) {
            //TODO 给VIEW添加方向
            focused.setTag(R.id.focus_change_direction, direction);
            result.setTag(R.id.focus_change_direction, direction);
            return result;
        }
